...
def parse_twilio_encoded_keys(url: str, response_body: str) -> None:
    ...
    for encoded_key in list(set(encoded_keys)):
        decoded_key = decode_base64(encoded_key)
        if decoded_key and VALID_DECODED_PATTERN.search(decoded_key):  # Check if the decoded key matches the valid pattern
            print(f'{url} - Encoded Key: {encoded_key}')
            print(f'{url} - Decoded Key: {decoded_key}')
            with open('twilio_encoded_keys.txt', 'a') as f:
                f.write(f'Encoded: {encoded_key}\n')
                f.write(f'Decoded: {decoded_key}\n')
            send_discord_webhook(url, "Twilio Encoded Key", decoded_key)
            matched = True

    if not matched:
        print(f'{url} - No valid encoded Twilio keys found')
